pipeline {
    ...
    stages {
        ...
        stage('Install Dependencies') {
            steps {
                sh 'npm ci'  // Changed from bat to sh
            }
        }
        
        stage('Lint') {
            steps {
                sh 'npm run lint'  // Changed from bat to sh
            }
        }
        
        stage('Test') {
            steps {
                sh 'npm test'  // Changed from bat to sh
            }
        }
        
        stage('Security Audit') {
            steps {
                sh 'npm audit --audit-level=moderate || exit 0'  // Changed from bat to sh
            }
        }
